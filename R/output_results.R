...
#' @export
output_results <- function(sfpca_data, model){
  basis <- basis_setup_sparse(sfpca_data, nknots = model$knot, orth=TRUE)
  rotation <- post_hoc_rotation(sfpca_data, model = model)
  ...
  if (('time_ori' %in% colnames(sfpca_data$data)) & ('response_ori' %in% colnames(sfpca_data$data))){
    var_require <- c('ID', 'time', 'time_ori', 'response', 'response_ori')
  } else {
    var_require <- c('ID', 'time', 'response')
  }
  vars_complete <- c(var_require, colnames(sfpca_data$data)[!colnames(sfpca_data$data) %in% var_require])
