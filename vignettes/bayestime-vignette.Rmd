...
```{r}
ECAM_remove <- ECAM[!duplicated(ECAM[, c('studyid', 'month_of_life')]), ]
library(data.table)
keys <- c(unique_subject_id, time)
X <- as.data.table(ECAM)
ECAM_mean <- X[,list(shannon_mean= mean(shannon)),keys]
ECAM_unique <- merge(ECAM_remove, ECAM_mean, by=c(unique_subject_id, time))
dat <- prepare_data(data = ECAM_unique, unique_subject_id = unique_subject_id,
                    time_name = time, response_name = response, 
                    transform_y = 'standardize', scale_time = T)
summary(dat)
```
...
p <- plot_mean_curve(output = model_output, original = T, x_lab = 'age', y_lab = 'shannon', ymin=0, ymax=7.6)
#dev.off()
p <- plot_mean_curve(output = model_output, original = F, x_lab = 'age', y_lab = 'shannon')
```

## plot_fpc_curve(): 
```{r, fig.height = 4, fig.width=6}
d <- plot_fpc_curve(output = model_output, pc_idx = c(1,2,3), original = T, x_tick = c(2,5,9))
d <- plot_fpc_curve(output = model_output, pc_idx = c(2,3), original = F)
```

## plot_fpc_on_mean_curve(): 
```{r, fig.height = 4, fig.width=6}
d <- plot_fpc_on_mean_curve(output = model_output, pc_idx = 1, original = T, sd = F)
     
d <- fpc_on_mean_curve_data2 <- plot_fpc_on_mean_curve(output = model_output, pc_idx = 2, original = F, sd = T)
```

## plot_fpc_group_mean():
```{r, fig.height = 4, fig.width=6}
d <- plot_fpc_group_mean(output = model_output, pc_idx = 1, original = T, group_name = 'delivery')
d <- plot_fpc_group_mean(output = model_output, pc_idx = 1, original = F, group_name = 'diet_3')
```
...
```{r}
invars <- invariants(ECAM_unique, unique_subject_id, time)
invars
```
