...
## plot_fpc_curve(): 
```{r}
plot_fpc_curve(output = model_output, pc_idx = c(1,2,3), ymin = -3, ymax = 5)
```
...
## plot_fpc_group_mean():
```{r}
plot_fpc_group_mean(model_output, pc_idx = 1, original = F, 'delivery')
```
...
```{r}
plot_fpc_boxplot(output = model_output, pc_idx = 2, group_name = 'diet_3')
```
