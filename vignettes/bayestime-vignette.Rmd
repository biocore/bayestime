...
We take ECAM data as an example to go through each function in BayesTime.
```{r}
data("ECAM")
head(ECAM)
```
...
BayesTime::plot_group() generates the spaghetti plot of interested variables by group. This takes the logitudinal data frame, the column name of time variable, column name of response variable, column name of unique subject id and a vector of column names of interested variables.
```{r}
...
## plot_density_overlay(): Plot the figure of overlay densities
...
```{r, fig.height = 4, fig.width=8}
plot_density_overlay(dat, optimal_model)
```
...
## output_results(): ***
```{r}
model_output <- output_results(sfpca_data = dat, variables = invars, basis = model_basis,
               rotation = model_rotation)
summary(model_output)
```

## sfpca_rda(): RDA analysis by fpc scores
BayesTime::sfpca_rda() automates the rda analysis using rda() from vegan package. This takes the output list from output_results(), stan model from optimal(), and a set of variables self-input or from invariants(). It returns the table and ggplot of the rda analysis results.
...
## plot_mean_curve(): ***
```{r}
plot_mean_curve(sfpca_data = dat, basis = model_basis, output = model_output,
                id_name = 'studyid', time_name = 'month_of_life',
                response_name = 'shannon', ymin=NULL, ymax=NULL)
```
